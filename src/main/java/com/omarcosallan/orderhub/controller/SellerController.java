...
public class SellerController {
    ...
    public ResponseEntity<SellerResponseDTO> create(@Valid @RequestBody SellerDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(sellerService.save(dto));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN') or (hasRole('SELLER') and @sellerService.isOwner(#id))")
